...
def _get_chat_room_by_by_users(user_id, to_user_id):
    ...
    chat_room = ChatRoomParticipants.objects.filter(user__id=to_user_id, room__id__in=chat_rooms).values_list('room__id', flat=True)
    if chat_room:
...
def get_chat_room_by_id(to_user_id,user_id):
    ...
    chat_room = ChatRoomParticipants.objects.filter(room__id=chat_id).exclude(user__id=user_id).values('room__id',
                                                                                                       'room__last_message',
                                                                                                       'room__last_sent_user__username',
                                                                                                       'room__last_sent_time',
                                                                                                       'user')
    if chat_room.exists():
        return chat_room.first()
